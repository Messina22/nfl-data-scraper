...
type Client struct {
	...
	Origin        string
	Cookie        string
	Authorization string // raw token; sent as Authorization (Action Network style, no Bearer)
}
...
func (c *Client) Get(ctx context.Context, url string) ([]byte, string, error) {
	...
	if c.Authorization != "" {
		// Action Network expects the JWT as the Authorization value with no "Bearer " prefix.
